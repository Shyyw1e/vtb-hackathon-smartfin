...
func (c *Client) Accounts(ctx context.Context, userID uuid.UUID) ([]banks.Account, error) {
	resp, err := c.req.Do(ctx, banks.Params{
		...
		NeedConsent: true, // у SBank часто pending — Requester вернёт ErrConsentPending
		UserID:      userID,
	})
	if err != nil {
		return nil, err
	...
	if resp.StatusCode/100 != 2 {
		return nil, httpError("sbank accounts", resp)
	}

	var ar AccountsResp
	if err := decodeJSON(resp.Body, &ar); err != nil {
		return nil, fmt.Errorf("sbank accounts decode: %w", err)
	}
...
func (c *Client) Transactions(ctx context.Context, userID uuid.UUID, since time.Time, limit int) ([]banks.Transaction, error) {
	...
	if resp.StatusCode/100 != 2 {
		return nil, httpError("sbank transactions", resp)
	}

	var tr TxResp
	if err := decodeJSON(resp.Body, &tr); err != nil {
		return nil, fmt.Errorf("sbank tx decode: %w", err)
	}

	out := make([]banks.Transaction, 0, len(tr.Flat()))
	for _, t := range tr.Flat() {
		amt, sign := moneyFromAny(t.Amount, t.Currency)
...
// DTO & util (идентичны по структуре)
